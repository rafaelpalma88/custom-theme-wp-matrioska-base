<div class="footer1 _relative">
  <div class="container">
    <div class="row">
      <div class="col-lg-6 col-md-6 col-12">
        <div class="single-footer-items footer-logo-area">
          <div class="footer-logo">
            <a href="<?php echo home_url('/'); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/logo_inverse_bottom.png" alt=""></a>
          </div>
          <div class="space20"></div>
          <div class="heading1-w">
            <p>Rafael Costa Palma Tecnologia da Informação Ltda<br />CNPJ 57.424.449/0001-02</p>
          </div>
          <ul class="social-icon">
            <li><a href="https://www.instagram.com/uruguaiplanob"><i class="fa-brands fa-instagram"></i></a></li>
          </ul>
        </div>
      </div>

      <div class="col-lg-6 col-md-6 col-12">
        <div class="single-footer-items">
          <h3>Entre em contato</h3>

          <div class="contact-box">
            <div class="icon">
              <img src="<?php echo get_template_directory_uri(); ?>/assets/img/icons/footer-icon1.png" alt="">
            </div>
            <div class="pera">
              <a href="tel:+5511994271484">+55 11 99427-1484</a>
            </div>
          </div>

          <div class="contact-box">
            <div class="icon">
              <img src="<?php echo get_template_directory_uri(); ?>/assets/img/icons/footer-icon2.png" alt="">
            </div>
            <div class="pera">
              <a href="mailto:contato@uruguaiplanob.com.br">contato@uruguaiplanob.com.br</a>
            </div>
          </div>
        </div>
      </div>

    </div>

    <div class="space70"></div>
  </div>

  <div class="copyright-area _relative">
    <div class="container">
      <div class="row align-items-center">
        <div class="col-md-12">
          <div class="coppyright">
            <p>© Copyright 2024 - Uruguai Plano B. Todos os direitos reservados.</p>
          </div>
        </div>
      </div>
    </div>

  </div>

</div>

<!--===== FOOTER AREA END =======-->

</body>

</html>

<script src="<?php echo get_template_directory_uri(); ?>/assets/js/bootstrap.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/aos.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/fontawesome.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/mobile-menu.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/jquery.magnific-popup.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/owl.carousel.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/slick-slider.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/gsap.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/jquery.countup.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/ScrollTrigger.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/Splitetext.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/SmoothScroll.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/text-animation.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/jquery.lineProgressbar.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/tilt.jquery.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/main.js"></script>